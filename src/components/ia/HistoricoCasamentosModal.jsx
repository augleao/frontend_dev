...
const categories = [
  { id: 'casamentosPago', label: 'Registros de Casamento Pago (7701, trib 01)', color: '#f97316' },
...
function SimpleLineChart({ data = [] }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Histórico mensal casamentos" style={{ width: '100%', height: 'auto' }}>
...
export default function HistoricoCasamentosModal({ open, onClose }) {
          ...
          p.then((res) => {
            ...
            if (res && res.full) {
              try {
                ...
                const counts = { casamentosPago: 0, casamentosGratis: 0, habilitacaoPaga: 0, habilitacaoGratis: 0 };
                periodos.forEach((pItem) => {
                  ...
                  atos.forEach((ato) => {
                    ...
                    if (code === '7701') {
                      if (tribNum === 1) counts.casamentosPago += qty;
                      if (tribNum === 11) counts.casamentosGratis += qty;
                    }
                    if (code === '7101') {
                      if (tribNum === 1) counts.habilitacaoPaga += qty;
                      if (tribNum === 11) counts.habilitacaoGratis += qty;
                    }
                  });
                });
                setHistorico((prev) => {
                  ...
                  next[idx].totals.casamentosPago = (next[idx].totals.casamentosPago || 0) + counts.casamentosPago;
                ...
                console.error('[HistoricoCasamentosModal] erro processando dap', { error: e, dap: res.dap });
              }
            } else if (res && res.error) {
              console.error('[HistoricoCasamentosModal] erro ao obter detalhes da DAP', { dap: res.dap, error: res.error });
            }
          }).catch((err) => { console.error('[HistoricoCasamentosModal] promise erro', err); });
        });

        await Promise.allSettled(promises);
      } catch (err) {
        console.error('[HistoricoCasamentosModal] erro ao carregar historico', err);
    ...
    <div style={{ position: 'fixed', top:0,left:0,right:0,bottom:0, background:'rgba(15,23,42,0.65)', display:'flex', alignItems:'center', justifyContent:'center', padding:24, zIndex:2100 }}>
      <div style={{ width: 'min(960px,100%)', background:'white', borderRadius:20, padding:24, boxShadow:'0 30px 60px rgba(15,23,42,0.35)' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
          <div>
            <h3 style={{ margin: 0 }}>Histórico Casamentos</h3>
            ...
            <p style={{ margin:'8px 0 0 0', fontSize:12, color:'#94a3b8' }}>Últimos {monthsRange} meses de registros de casamento e habilitação por tributação.</p>
          </div>
