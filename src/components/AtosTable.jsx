...
export default function AtosTable({ texto }) {
  ...
  const inputStyleCaixa = { width: '130px', marginRight: '12px' };
  const labelStyle = { fontSize: '0.75rem', marginRight: '4px', display: 'inline-block', width: '30px' };
  const brStyle = { display: 'block', height: '4px' };
  const caixaContainerStyle = { display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '12px', flexWrap: 'wrap' };
...
return (
  ...
  {atosComISS.map(ato => {
    ...
    let linhaClass = 'atos-table-row';
    if (Math.abs(somaPagamentos - ato.valorTotalComISS) < 0.01) {
      linhaClass = 'atos-table-row success-row';
    } else {
      linhaClass = 'atos-table-row error-row';
    }
    return (
      <tr
        key={ato.id}
        className={linhaClass}
        style={{ display: 'table-row', borderBottom: '1px solid #e0e0e0' }}
      >
