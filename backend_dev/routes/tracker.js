...
async function ensureTable() {
  ...
  async function processTrackerEvent(pool, body, req) {
    ...
    const hashed = pseudonymize(rawUid);
      ...
      if (TRACKER_DEBUG) {
        try {
          console.warn('[tracker][debug] incoming payload', {
            rawUid,
            hashed,
            bodyData: body && body.data,
            cookie_track_uid: req && req.cookies ? req.cookies.track_uid : undefined,
            auth_present: req && req.headers ? Boolean(req.headers.authorization) : false,
            ip: clientIp(req)
          });
        } catch (e) {}
      }
