...
const { ensureAuth, authenticate } = require('../middlewares/auth');
...
function registerIaRoutes(app) {
  ...
  app.get('/api/ia/health', ensureAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          if (!rowTpl || !rowTpl.prompt) {
            return res.status(422).json({ error: 'Prompt "extrair_texto" não configurado no banco.' });
          }
          const promptText = renderTemplate(rowTpl.prompt, { texto: clampForPrompt(text) });
  ...
  app.post('/api/ia/analise-mandado', upload3.single('file'), async (req, res) => {
    try {
      ...
      // If PDF appears to have no extractable text, attempt OCR using system tools (pdftoppm + tesseract)
      if (!text || text.trim().length < 15) {
        const fs = require('fs');
        const os = require('os');
        const path = require('path');
        const child_process = require('child_process');
        const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'biblio-ocr-'));
        const pdfPath = path.join(tmpDir, 'upload.pdf');
        try {
          fs.writeFileSync(pdfPath, req.file.buffer);
          // Try pdftoppm (poppler) first to convert PDF pages to PNGs
          const outPrefix = path.join(tmpDir, 'page');
          try {
            await new Promise((resolve, reject) => {
              child_process.execFile('pdftoppm', ['-png', '-r', '300', pdfPath, outPrefix], (err) => (err ? reject(err) : resolve()));
            });
          } catch (ppErr) {
            // Fallback: try ImageMagick `magick` to convert PDF to PNGs
            try {
              await new Promise((resolve, reject) => {
                child_process.execFile('magick', ['-density', '300', pdfPath, path.join(tmpDir, 'page-%03d.png')], (err) => (err ? reject(err) : resolve()));
              });
            } catch (magErr) {
              throw new Error('Nenhuma ferramenta de conversão de PDF para imagem disponível (pdftoppm ou magick).');
            }
          }

          // Collect generated images
          const files = fs.readdirSync(tmpDir).filter(f => f.toLowerCase().endsWith('.png')).sort();
          if (!files || files.length === 0) throw new Error('Nenhuma imagem gerada para OCR.');

          // Run tesseract on each image (language: Portuguese)
          let ocrCombined = '';
          for (const f of files) {
            const imgPath = path.join(tmpDir, f);
            try {
              const ocrOut = child_process.execFileSync('tesseract', [imgPath, 'stdout', '-l', 'por'], { encoding: 'utf8', maxBuffer: 20 * 1024 * 1024 });
              ocrCombined += String(ocrOut || '') + '\n';
            } catch (tErr) {
              // If tesseract failed for a page, continue with others
              try { console.warn('[IA][analise-mandado][OCR] tesseract failed for', imgPath, tErr && tErr.message ? tErr.message : tErr); } catch(_){}
            }
          }
          text = normalizeText(ocrCombined || '');
          pages = files.length || pages;
          try { console.log('[IA][analise-mandado][OCR] extracted len=', (text || '').length, 'pages=', pages); } catch(_){}
        } catch (ocrErr) {
          try { console.warn('[IA][analise-mandado] OCR fallback failed:', ocrErr && ocrErr.message ? ocrErr.message : ocrErr); } catch(_){}
        } finally {
        ...
        if (!text || text.trim().length < 15) {
          ...
          return res.status(422).json({ error: 'PDF sem texto extraível (provavelmente escaneado ou protegido). Foi tentado OCR sem sucesso.' });
        }
      }
  ...
  // Identificar tipo
  // Allow any authenticated user (não apenas Registrador)
  app.post('/api/ia/identificar-tipo', authenticate, async (req, res) => {
    try {
  ...
  const inNonWS = typeof text === 'string' ? (text.replace(/\s+/g, '').length) : 0;
      ...
      try {
        ...
        if (!rowTpl || !rowTpl.prompt) {
          return res.status(422).json({ error: 'Prompt "identificar_tipo_mandado" não configurado no banco.' });
        }
        const prompt = renderTemplate(rowTpl.prompt, { texto: text.slice(0, 8000) });
        const t0 = Date.now();
        const { resp: provResp, usedModel } = await generateContentWithFallback(genAI, modelCandidates, prompt, { retries: 3, baseDelayMs: 700 });
      const out = (provResp && provResp.response && provResp.response.text && provResp.response.text()) || '';
  ...
  app.post('/api/ia/analisar-exigencia', ensureAuth, async (req, res) => {
    try {
      ...
      try {
        ...
        if (!rowTpl || !rowTpl.prompt) {
          return res.status(422).json({ error: 'Prompt "analisar_mandado" não configurado no banco.' });
        }
        const prompt = renderTemplate(rowTpl.prompt, { tipo: tipo || 'n/d', texto: text.slice(0, 8000), legislacao_bullets: contextoLegal });
    const { resp: provResp, usedModel } = await generateContentWithFallback(genAI, modelCandidates, prompt, { retries: 3, baseDelayMs: 700 });
      const out = (provResp && provResp.response && provResp.response.text && provResp.response.text()) || '';
  ...
  app.post('/api/ia/gerar-texto-averbacao', ensureAuth,async (req, res) => {
    try {
      ...
      try {
        ...
        if (!rowTpl || !rowTpl.prompt) {
          return res.status(422).json({ error: 'Prompt "criar_averbacao" não configurado no banco.' });
        }
        const prompt = renderTemplate(rowTpl.prompt, { tipo: tipo || 'n/d', texto: text.slice(0, 8000), legislacao_bullets: contextoLegal });
    const { resp: provResp, usedModel } = await generateContentWithFallback(genAI, modelCandidates, prompt, { retries: 3, baseDelayMs: 700 });
      const out = (provResp && provResp.response && provResp.response.text && provResp.response.text()) || '';
