...
module.exports = function initDapRoutes(appArg = null, poolArg = null, options = {}) {
  ...
  const enforceServentia = options.enforceServentia || (async (req) => {
    const user = req.user || {};
    const filters = extractFilters(req.query || {});

    const candidateSet = new Set();

    // 1) Priorizar exatamente o que a listagem usa: query `codigoServentia`/`codigo_serventia`
    const directCode = filters.codigoServentia || req.query?.codigo_serventia;
    if (directCode) candidateSet.add(String(directCode));
    ...
    userCodes.forEach((v) => candidateSet.add(String(v)));
    ...
    // 2) Fallback: buscar pelo nome salvo no user na tabela DAP para recuperar o código mais recente
    if (candidateSet.size === 0 && (user.serventia || user.serventiaNome)) {
      const nome = user.serventia || user.serventiaNome;
      try {
        const lookupSql = `
          SELECT codigo_serventia
          FROM public.dap
          WHERE lower(serventia_nome) LIKE lower($1)
            AND codigo_serventia IS NOT NULL
          ORDER BY id DESC
          LIMIT 1
        `;
        const { rows } = await db.query(lookupSql, [`%${nome}%`]);
        if (rows?.length && rows[0].codigo_serventia) {
          candidateSet.add(String(rows[0].codigo_serventia));
          try { console.log('[historico-nas-ob] servantia from DAP name fallback', rows[0].codigo_serventia); } catch (_) {}
        }
        // Mesmo que não ache código, guarde o texto para consulta por nome
        candidateSet.add(String(nome));
      } catch (e) {
        try { console.error('[historico-nas-ob] fallback lookup error', e.message); } catch (_) {}
        candidateSet.add(String(nome));
      }
    }

    // 3) Último fallback: tentar pegar `serventia` texto da tabela users (se houver)
    if (candidateSet.size === 0 && req.user && req.user.id && db) {
      try {
        const q = 'SELECT serventia FROM public.users WHERE id = $1 LIMIT 1';
        const { rows } = await db.query(q, [req.user.id]);
        if (rows && rows[0] && rows[0].serventia) {
          const nome = String(rows[0].serventia);
          candidateSet.add(nome);
          try { console.log('[historico-nas-ob] servantia from users table (texto)', nome); } catch (_) {}
          // tenta achar código correspondente em DAP pelo nome textual
          try {
            const lookupSql = `
              SELECT codigo_serventia
              FROM public.dap
              ...
              LIMIT 1
            `;
            const { rows: r2 } = await db.query(lookupSql, [`%${nome}%`]);
            if (r2?.length && r2[0].codigo_serventia) {
              candidateSet.add(String(r2[0].codigo_serventia));
            }
          } catch (e2) {
            try { console.error('[historico-nas-ob] users-text fallback lookup error', e2.message); } catch (_) {}
          }
        }
      } catch (e) {
    ...
    const candidates = Array.from(candidateSet).filter((v) => v && String(v).trim() !== '');
    ...
    // Guarda candidatos para reutilizar na rota de histórico
    req._serventiaCandidates = candidates;
  ...
  app.get('/api/dap/historico-nas-ob', guard, async (req, res) => {
    try {
      const codigoServentia = await enforceServentia(req);
      ...
      // Usa candidatos resolvidos pelo enforceServentia (códigos ou texto) para abranger códigos e nomes
      const rawCandidates = (Array.isArray(req._serventiaCandidates) && req._serventiaCandidates.length
        ? req._serventiaCandidates
        : [String(codigoServentia)]).map((c) => String(c));
      const codeCandidates = rawCandidates.filter((c) => /\d/.test(c));
      const codeCandidatesNormalized = codeCandidates.map((c) => c.replace(/\D/g, '')).filter((v) => v.length > 0);
      const nameCandidates = rawCandidates.filter((c) => !/\d/.test(c)).map((s) => String(s));
      const nameCandidatesExact = nameCandidates.map((s) => s.toLowerCase());
      const nameCandidatesLike = nameCandidates.map((s) => `%${String(s).toLowerCase()}%`);
      try {
        console.log('[historico-nas-ob] candidates', {
          rawCandidates,
          codeCandidates,
          codeCandidatesNormalized,
          nameCandidates,
          nameCandidatesExact,
          nameCandidatesLike,
        });
      ...
      // Diagnóstico: contagens de DAP por cada critério dentro da janela
      try {
        const diagSql = `
          SELECT
            COUNT(*) FILTER (WHERE (ano_referencia * 100 + mes_referencia) >= $1) AS total_window,
            COUNT(*) FILTER (WHERE (ano_referencia * 100 + mes_referencia) >= $1 AND codigo_serventia = ANY($2)) AS match_codigo_raw,
            COUNT(*) FILTER (WHERE (ano_referencia * 100 + mes_referencia) >= $1 AND regexp_replace(codigo_serventia, '\\D', '', 'g') = ANY($3)) AS match_codigo_norm,
            COUNT(*) FILTER (WHERE (ano_referencia * 100 + mes_referencia) >= $1 AND lower(serventia_nome) = ANY($4)) AS match_nome_eq,
            COUNT(*) FILTER (WHERE (ano_referencia * 100 + mes_referencia) >= $1 AND lower(serventia_nome) LIKE ANY($5)) AS match_nome_like
          FROM public.dap`;
        const { rows: diag } = await db.query(diagSql, [
          monthThreshold,
          codeCandidates,
          codeCandidatesNormalized,
          nameCandidatesExact,
        ...
        try { console.log('[historico-nas-ob] diag', diag && diag[0]); } catch (_) {}
      } catch (e) {
        try { console.log('[historico-nas-ob] diag error', e.message); } catch (_) {}
      }
      ...
      const sql = `
        ...
        WHERE (
            d.codigo_serventia = ANY($1)
         OR regexp_replace(d.codigo_serventia, '\\D', '', 'g') = ANY($2)
         OR lower(d.serventia_nome) = ANY($3)
         OR lower(d.serventia_nome) LIKE ANY($4)
        )
          AND (d.ano_referencia * 100 + d.mes_referencia) >= $5
          AND ato.codigo = ANY($6)
          AND CAST(NULLIF(ato.tributacao, '') AS INTEGER) = ANY($7)
