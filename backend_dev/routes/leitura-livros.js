...
async function ocrAndExtractRecords(filePaths, params, status, cancelFlagRef, ctx = {}) {
    ...
    try {
      ...
      if (fext === '.p7s') {
        try {
          ...
          if (payload && payload.buffer) {
            ...
            if (detected.type === 'pdf') {
              ...
              try { extractedParts.push({ file: path.basename(outPath), text: String(text || '') }); } catch(_){}
        ...
        try {
          ...
          try { extractedParts.push({ file: path.basename(first), text: String(text || '') }); } catch(_){}
  ...
  for (const fp of filePaths) {
    ...
    try {
      ...
      if (ext === '.p7s') {
        ...
        if (payload && payload.buffer) {
          ...
          const paired = findPairForDetachedP7s(fp, filePaths);
            ...
            if (pext === '.pdf') {
              ...
              try { extractedParts.push({ file: path.basename(paired), text: String(text || '') }); } catch(_){}
        ...
        try { extractedParts.push({ file: path.basename(fp), text: String(text || '') }); } catch(_){}
  ...
  if (manuscriptImages.length && !cancelFlagRef.cancelled) {
    ...
    if (aggregatedText && aggregatedText.trim().length) {
      try { extractedParts.push({ file: 'aggregated_manuscripts.txt', text: aggregatedText }); } catch(_){}
    }
...
async function runJob(jobId, inputs, params) {
  ...
  const MAX_INLINE = Number(process.env.LEITURA_MAX_FULLTEXT_INLINE_BYTES || 200000);
  const extractionResult = await ocrAndExtractRecords(filePaths, params, status, cancelFlag, ctx);
  let records = Array.isArray(extractionResult) ? extractionResult : (extractionResult && Array.isArray(extractionResult.records) ? extractionResult.records : []);
  const extractedParts = extractionResult && Array.isArray(extractionResult.extractedParts) ? extractionResult.extractedParts : [];
  // Build fullText and persist to job dir
  let fullText = '';
  try {
    if (extractedParts.length) {
      fullText = extractedParts.map(p => `--- ${p.file} ---\n${p.text || ''}`).join('\n\n');
      const fullPath = path.join(paths.dir, 'fulltext.txt');
      await fsp.writeFile(fullPath, fullText, 'utf8');
    }
  } catch (e) {
    pushMessage(status, 'warning', `Falha ao gravar fulltext: ${e && e.message ? e.message : e}`);
  }
  ...
  try {
    ...
    const result = {
      ...
      fullTextPath: fullText && fullText.length ? 'fulltext.txt' : null,
      fullTextPreview: fullText && fullText.length ? (fullText.length > 2000 ? fullText.slice(0,2000) + ' …[truncado]' : fullText) : null,
      fullTextInline: (fullText && fullText.length && Buffer.byteLength(fullText, 'utf8') <= MAX_INLINE) ? fullText : null,
      status: {
        state: status.status,
...
function registerLeituraLivrosRoutes(app) {
  const registerBase = (base = '') => {
    ...
    try {
        ...
        // Attach fullText information (preview/inline/download link) when available in result.json
        try {
          payload.fullTextPreview = result.fullTextPreview || null;
          payload.fullTextInline = result.fullTextInline || null;
          payload.fullTextAvailable = !!(result.fullTextPath || result.fullTextInline);
          payload.fullTextDownload = result.fullTextPath ? `${base}/leitura-livros/fulltext/${jobId}` : null;
        } catch (_) {}
    ...
    // Fulltext download (inteiro teor) — arquivo salvo em jobDir/fulltext.txt
    app.get(`${base}/leitura-livros/fulltext/:jobId`, authenticate, async (req, res) => {
    const { jobId } = req.params;
    const paths = jobPaths(jobId);
    try {
      const filePath = path.join(paths.dir, 'fulltext.txt');
      ...
      return res.status(404).json({ error: 'fulltext não encontrado' });
    }
    });
